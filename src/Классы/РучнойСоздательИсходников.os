...
Функция Добавить_Язык()
	...
	путь_Языка = ОбъединитьПути("Languages","Русский.xml");	
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "Languages"));
	...
	ЗаписатьФайлПоПути(ШаблоныФайловКонфигурации.ЗаполнитьШаблонПараметрами(Шаблон, СтруктураПараметров), ОбъединитьПути(ПутьККаталогу,путь_Языка));
...
Функция Добавить_Роль()
	...
	путь_Роли = ОбъединитьПути("Roles",РОЛЬ_ИМЯ + ".xml");	
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "Roles"));
	...
	ЗаписатьФайлПоПути(ШаблоныФайловКонфигурации.ЗаполнитьШаблонПараметрами(Шаблон, СтруктураПараметров), ОбъединитьПути(ПутьККаталогу, путь_Роли));
...
Функция Добавить_Сервис()
	...
	ПутьСервиса = ОбъединитьПути("HTTPServices", ПолноеИмяСервиса + ".xml");
	
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "HTTPServices"));
	...
	ЗаписатьФайлПоПути(ШаблоныФайловКонфигурации.ЗаполнитьШаблонПараметрами(Шаблон_Сервис, СтруктураСервиса), ОбъединитьПути(ПутьККаталогу, ПутьСервиса)); 
...
Процедура Добавить_Сервис_Модуль(Знач СтруктураВходящихПарметров)
	...
	ПутьМодуля = ОбъединитьПути("HTTPServices", СЕРВИС_ИМЯ, "Ext","Module.bsl");
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "HTTPServices", СЕРВИС_ИМЯ));
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "HTTPServices", СЕРВИС_ИМЯ, "Ext"));
	
	ЗаписатьФайлПоПути(ОписаниеМодуля, ОбъединитьПути(ПутьККаталогу, ПутьМодуля));
	
КонецПроцедуры

Функция Добавить_ОбщийМодуль(ТипМодуля, Знач СтруктураСервиса)
	
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "CommonModules"));
	
	МОДУЛЬ_ИМЯ = ПрефиксРасширения + ТипМодуля;
	МОДУЛЬ_УИД = Строка(Новый УникальныйИдентификатор);
	
	путь_Модуля = ОбъединитьПути("CommonModules", МОДУЛЬ_ИМЯ + ".xml");	
	...
	ЗаписатьФайлПоПути(РезультатЗаполнения, ОбъединитьПути(ПутьККаталогу, путь_Модуля)); 
...
Процедура Добавить_ОбщийМодуль_Модуль(Знач СтруктураПараметров, СтруктураСервиса)
	...
	ПутьМодуля = ОбъединитьПути("CommonModules", МОДУЛЬ_ИМЯ, "Ext","Module.bsl");
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "CommonModules", МОДУЛЬ_ИМЯ));
	СоздатьКаталог(ОбъединитьПути(ПутьККаталогу, "CommonModules", МОДУЛЬ_ИМЯ, "Ext"));
...
Функция Добавить_Конфигурацию(Знач СтруктураВходящихПарметров)
	...
	ЗаписатьФайлПоПути(ШаблоныФайловКонфигурации.ЗаполнитьШаблонПараметрами(ШаблонКонфигурации, СтруктураПараметров), ОбъединитьПути(ПутьККаталогу, путь_Шаблона));
...
Процедура Добавить_ОписаниеЗагрузки(Знач СтруктураВходящихПарметров)
	...
	ЗаписатьФайлПоПути(ШаблонОписанияЗагрузки, ОбъединитьПути(ПутьККаталогу, путь_Шаблона));	
